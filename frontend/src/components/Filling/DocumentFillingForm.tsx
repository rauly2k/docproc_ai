...
export const DocumentFillingForm: React.FC = () => {
  ...
  const downloadMutation = useMutation({
    mutationFn: () => apiClient.getFilledPdfDownloadUrl(selectedIdDoc),
    onSuccess: (data: any) => {
      ...
      <div className="space-y-4">
        ...
        {fillMutation.isSuccess && (
          <div className="bg-green-50 border border-green-400 text-green-700 px-4 py-3 rounded">
            <p className="font-medium">Document filling started!</p>
            <p className="text-sm mt-1">Processing... The filled PDF will be ready shortly.</p>
          </div>
        )}

        {/* Download Button */}
        {filledDocId && fillMutation.isSuccess && (
          <div className="pt-4 border-t">
            ...
            {downloadMutation.isError && (
              <p className="text-sm text-red-600 mt-2">
                Document may still be processing. Please try again in a few seconds.
              </p>
            )}
          </div>
        )}
